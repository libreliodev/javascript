			...
			(function() {
				...
				var box = $('#dialog_box'),
					...
					dbox = function(type, msg, callback, defaultVal, opts, changeCb, checkbox) {
						var ok, ctrl, chkbx;
                        if(type == 'dialog')
                            dialog_content.html('').append(msg);
                        else
						...
						if (type !== 'alert') {
							$('<input type="button" value="' + uiStrings.common.cancel + '">')
								.appendTo(btn_holder)
								.click(function() { box.hide(); callback && callback(false);});
						}
                        ...
                        if(type == 'dialog')
                        {
                            ...
                            {
						        btn_holder.empty();
                                for(var btn_name in callback)
                                    $('<input type="button" value="' + 
                                      btn_name + '">').appendTo(btn_holder)
                                         .click(callback[btn_name]);
                            }
                ...
                $.open_dialog = function(html, cb) { dbox('dialog', html, cb);};
...
//			});
			...
			//	var revnums = "svg-editor.js ($Rev: 2839 $) ";
